...
public class PropTest {
	...
	public static void fileSave(Properties p) throws IOException {
		...
		p.setProperty("2", "bnana,2500,2");
		p.setProperty("3", "grape,4500,5");
		p.setProperty("4", "orange,800,10");
		p.setProperty("5", "melon,5000,2");
		
		//Properties 객체를 XML 파일로 저장
		FileOutputStream fos = new FileOutputStream("data.xml"); // data.xml파일에 출력하겠다.
		p.storeToXML(fos, null);
		//xml 파일로 저장할 것 fos : 입출력 입력하겠다. null 코멘트로 설명을 달아서 저장하겠다 라는 것
	...
	public static void fileOpen(Properties p) {
		try {
			...
			//파일에 저장
			for(String key : p.stringPropertyNames()) {//설정에 적혀있는 문자이름을 가져와서 key로 넣을 예정
